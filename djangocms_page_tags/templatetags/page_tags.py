...
class IncludePageTagsList(InclusionTag):
    ...
    def get_context(self, context, page_lookup, lang, site):
        ...
        if not request:  # pragma: no cover
            return {"tags_list": ""}
        tags_list = get_page_tags_from_request(request, page_lookup, lang, site, self.title)
        if tags_list:
...
class PageTagsList(AsTag):
    ...
    def get_value(self, context, page_lookup, lang, site):
        ...
        if not request:  # pragma: no cover
            return ""
        return get_page_tags_from_request(request, page_lookup, lang, site, self.title)
